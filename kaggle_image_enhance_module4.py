...
# this is giving nearly proper result for image pre process
image_path = "images/media.jpg"
bw_img = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
bw_img_thresh = cv2.adaptiveThreshold(bw_img, maxValue=255, adaptiveMethod=cv2.ADAPTIVE_THRESH_MEAN_C, thresholdType=cv2.THRESH_BINARY, blockSize=21, C=2)
...
save_path = 'images/thresholded_image_' + image_path.split('/')[-1]  # Extracting just the file name
cv2.imwrite(save_path, bw_img_thresh)
